...
describe('SessionStatsContext', () => {
    ...
    it('should aggregate token usage correctly when addUsage is called', () => {
        ...
        expect(stats?.cumulative.totalTokenCount).toBe(mockMetadata1.totalTokenCount ?? 0);
        expect(stats?.cumulative.promptTokenCount).toBe(mockMetadata1.promptTokenCount ?? 0);
        expect(stats?.cumulative.apiTimeMs).toBe(123);
        // Check that turn count is NOT incremented
        expect(stats?.cumulative.turnCount).toBe(0);
        // Check that currentTurn is updated
        expect(stats?.currentTurn?.totalTokenCount).toEqual(mockMetadata1.totalTokenCount);
    ...
    it('should throw an error when useSessionStats is used outside of a provider', () => {
        ...
        render(_jsx(TestHarness, { contextRef: contextRef }));
        ...
        expect(errorSpy.mock.calls[0][0]).toContain('useSessionStats must be used within a SessionStatsProvider');
